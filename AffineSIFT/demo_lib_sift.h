...
{
public:
	dmatching() : matching(), distance(0.0), queryIdx(0), trainIdx(0) {}
	dmatching(matching m, double distance, int queryIdx, int trainIdx)
		: matching(m), distance(distance), queryIdx(queryIdx), trainIdx(trainIdx) {}
	matching matching;
	double distance;
	int queryIdx;
	int trainIdx;
...
typedef std::vector<dmatching> matchingslist;
