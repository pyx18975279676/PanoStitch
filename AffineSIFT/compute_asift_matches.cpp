  ...
  if ( seg_in.size() > 1 )	
    {
      /* check if a match is unique. if yes, copy */
		...
		*/
		...
		for ( i_in = 1; i_in < (int) seg_in.size(); i_in++, ptr_in++ )
			{				
			  x1_in = ptr_in->matching.first.x;
			  y1_in = ptr_in->matching.first.y;
			  x2_in = ptr_in->matching.second.x;
			  y2_in = ptr_in->matching.second.y;
			  ...
			  for ( i_out = 0; i_out < (int) seg_out.size(); i_out++, ptr_out++ )
				{
					x1_out = ptr_out->matching.first.x;
					y1_out = ptr_out->matching.first.y;
					x2_out = ptr_out->matching.second.x;
					y2_out = ptr_out->matching.second.y;
...
{  	
	...
	// Fix by Miguel Colom (http://mcolom.info), Oct. 30, 2018.
	//
	// Compute a good number of threads to avoid an overflow.
	// If we're using too many threads, openMP will stop the program with this message:
	// "libgomp: Thread creation failed: Resource temporarily unavailable".
	unsigned num_procs = 8;// omp_get_num_procs();
	unsigned T1 = num_procs / 6;
	unsigned T2 = num_procs / 16;
	unsigned T3 = num_procs / 8;
	unsigned T4 = num_procs / 10;
	//
	if (T1 <= 0) T1 = 1;
	if (T2 <= 0) T2 = 1;
	if (T3 <= 0) T3 = 1;
	...
	// loop on tilts for image 1. 
#pragma omp parallel for private(tt) num_threads(T1)
	for (int tt = 1; tt <= num_tilt1; tt++)
	{
		...
		// Loop on rotations for image 1. 
#pragma omp parallel for private(rr) num_threads(T2)
		for  ( int rr = 1; rr <= num_rot1; rr++ ) 
		{   
			...
			// loop on tilts for image 2.
#pragma omp parallel for private(tt2) num_threads(T3)
			for (int tt2 = 1; tt2 <= num_tilt2; tt2++)				
			{
				float t_im2 = t_min * pow(t_k, tt2-1);
...
#pragma omp parallel for private(rr2) num_threads(T4)
				// Loop on rotations for image 2. 
				for  ( int rr2 = 1; rr2 <= num_rot1_2; rr2++ ) 
				{   
	...
	if ( matchings.size() > 0 )
	{
	  /* Remove the repetitive matches that appear in different simulations and retain only one. */		
		...
		{			
			...
			{
				Match match1_coor;
				match1_coor.x1 = matchings[cc].matching.first.x;
				match1_coor.y1 = matchings[cc].matching.first.y;
				match1_coor.x2 = matchings[cc].matching.second.x;
				match1_coor.y2 = matchings[cc].matching.second.y;
