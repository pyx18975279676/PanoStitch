...
static float InitSigma_aa = 1.6f;
...
void compensate_affine_coor1(float *x0, float *y0, int w1, int h1, float t1, float t2, float Rtheta)
{
	...
	else
	{
		...
		y_ori = (float)((w1 * sin(Rtheta) + h1 * sin(Rtheta - PI / 2)) / t1);
	}
...
// Output: the number of keypoints 
{
	...
	int flag_dir = 1;
	float BorderFact = (float)(6 * sqrt(2.));
...
#pragma omp parallel for private(tt)
	for (tt = 1; tt <= num_tilt; tt++)
	{
		...
		else
		{
			...
			num_rot1 = num_rot1 / 2;
			float delta_theta = (float)(PI / num_rot1);
...
#pragma omp parallel for private(rr)
			for (int rr = 1; rr <= num_rot1; rr++)
			{
				...
				float fproj_x1 = 0;
				float fproj_y1 = 0;
				float fproj_x2 = (float)width_t;
				float fproj_y2 = 0;
				float fproj_x3 = 0;
				float fproj_y3 = (float)height_t;
