...
int compute_asift_keypoints(vector<float>& image,	//图像数据
	int width,	//图像宽度
	int height,	//图像高度
	int num_of_tilts,	//模拟的级数（number N of tilts to simulate t = 1, \sqrt{2}, (\sqrt{2})^2, ..., {\sqrt{2}}^(N-1)）
	int verb,	//是否（1/0）打印信息
	vector< vector< keypointslist > >& keys_all,	//检测出来的特征点
	siftPar &siftparameters	//SIFT运行参数
	);
